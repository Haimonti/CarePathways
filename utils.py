...
def build_structured_input(form_data: dict):
    parts = []
...
def chunk_long_section(text: str, name: str, tokenizer, budget: int):
...
def build_chunk_text(chunk_sections: list):
    parts = [f"<{name}> {text}" for name, text in chunk_sections]
    return T5_PROMPT + " </s> ".join(parts)

def make_cache_key(text: str, model_name: str):
...
def save_cached(path: str, features):
