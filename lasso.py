...
# FILE = "/projects/academic/haimonti/skhaniza/BOW-CC/"+args.split_name
FILE = args.split_dir
...
for a in ALPHAS:
    m = Lasso(alpha=float(a), max_iter=MAX_ITER).fit(X_tr_s, y_tr)
...
final = Lasso(alpha=best_a, max_iter=MAX_ITER).fit(sx2.transform(X_trv), y_trv)
